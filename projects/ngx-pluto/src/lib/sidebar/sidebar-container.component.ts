 ...
 */
@Component({
  selector: 'np-sidebar-container',
  template: `
    <div *ngIf="showBackdrop"
      aria-hidden="true"
      class="np-sidebar__backdrop"
      [ngClass]="backdropClass"
      (click)="_onBackdropClicked()"></div>

    <ng-content select="np-sidebar,[np-sidebar]"></ng-content>

    <div class="np-sidebar__content"
      [class.np-sidebar__content--animate]="animate"
      [ngClass]="contentClass"
      [ngStyle]="_getContentStyle()">
      <ng-content select="[np-sidebar-content]"></ng-content>
    </div>
  `,
  styles: [`
    :host {
...
export class NpSidebarContainer implements AfterContentInit, OnChanges, OnDestroy {
  ...
  private _sidebars: Array<NpSidebar> = [];
   ...
   */
  _removeSidebar(sidebar: NpSidebar) {
   ...
   */
  private _subscribe(sidebar: NpSidebar): void {
