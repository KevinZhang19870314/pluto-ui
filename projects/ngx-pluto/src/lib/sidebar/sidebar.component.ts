...
@Component({
  ...
  template: `
    <aside #sidebar
      ...
      class="np-sidebar np-sidebar--{{opened ? 'opened' : 'closed'}} np-sidebar--{{position}} np-sidebar--{{mode}}"
      [class.np-sidebar--docked]="_isDocked"
      [class.np-sidebar--inert]="_isInert"
      [class.np-sidebar--animate]="animate"
...
export class NpSidebar implements OnInit, OnChanges, OnDestroy {
  ...
  constructor(@Optional() private _container: NpSidebarContainer, private _ref: ChangeDetectorRef) {
    if (!this._container) {
      throw new Error(
        '<np-sidebar> must be inside a <np-sidebar-container>. '
      );
    }
   ...
   *
   * return The transform styles, with the WebKit-prefixed version as well.
   */
